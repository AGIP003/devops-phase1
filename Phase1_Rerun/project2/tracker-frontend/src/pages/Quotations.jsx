...
function SupplierForm({ items, initialQuote, saving, onCancel, onSubmit }) {
  ...
  return <form className="quote-editor quote-supplier-editor" onSubmit={(event) => { event.preventDefault(); onSubmit(quotePayload(form)); }}>
    ...
    <button type="button" className="quote-details-toggle" aria-expanded={showDetails} onClick={() => setShowDetails((visible) => !visible)}>{showDetails ? "Hide optional quotation details" : "Add delivery, discount, tax or payment terms"}</button>
