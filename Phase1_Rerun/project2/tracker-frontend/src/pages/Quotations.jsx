...
import { Check, CircleAlert, Clock3, Pencil, Plus, Search, Trash2, Truck, X } from "lucide-react";
...
function ItemForm({ saving, onCancel, onSubmit }) {
  const [form, setForm] = useState({ name: "", quantity: "1", unit: "pcs" });
  return <form className="quote-editor quote-compact-editor" onSubmit={(event) => { event.preventDefault(); onSubmit(form); }}>
    <EditorHeading eyebrow="Requested item" title="Add the same requirement for every supplier" onClose={onCancel} />
    <div className="quote-form-grid quote-item-form-grid">
      ...
      <Field label="Unit"><input required maxLength={30} value={form.unit} onChange={(event) => setForm({ ...form, unit: event.target.value })} /></Field>
    </div>
    <div className="quote-form-actions"><button type="button" className="quote-secondary-button" onClick={onCancel}>Cancel</button><button className="feature-primary-button" disabled={saving}>{saving ? "Adding…" : "Add item"}</button></div>
  </form>;
}
...
function SupplierForm({ items, initialQuote, saving, onCancel, onSubmit }) {
  ...
  return <form className="quote-editor quote-supplier-editor" onSubmit={(event) => { event.preventDefault(); onSubmit(quotePayload(form)); }}>
    ...
    <div className="quote-form-grid">
      ...
      <Field label="Valid until"><input required type="date" value={form.validUntil} onChange={(event) => setValue("validUntil", event.target.value)} /></Field>
    ...
    <div className="quote-form-grid quote-cost-grid">
...
function QuoteCard({ quote, lowestId, formatCurrency, saving, onEdit, onDelete, onPrefer }) {
  ...
  return <article className={`quote-offer-card${quote.preferred ? " is-preferred" : ""}`}>
    ...
    <dl className="quote-offer-facts">
      ...
      <div><dt><Clock3 size={15} /> Validity</dt><dd className={expired ? "is-expired" : ""}>{expired ? `Expired ${formatDate(quote.validUntil)}` : `Until ${formatDate(quote.validUntil)}`}</dd></div>
...
function Quotations() {
  ...
  async function mutate(request, message) {
    ...
    try {
      ...
      setSelectedId(updated.id); setPanel(null); setEditingQuote(null); toast.success(message);
    } catch (requestError) { setError(requestError.message || "That change could not be saved"); }
  ...
  return <div className="feature-page quotation-page">
    <header className="feature-page-header quotation-page-header"><div><span className="coming-soon-pill">Supplier decisions</span><h1>Quotation comparison</h1><p>Compare the real delivered cost—not just the biggest number on a supplier’s page.</p></div><button type="button" className="feature-primary-button" onClick={() => setPanel("project-create")}><Plus size={17} /> New comparison</button></header>
    ...
    <div className="quote-workspace">
      ...
      <main className="quote-main">
        ...
        {!loading && project && <>
          ...
          {panel === "item" && <ItemForm saving={saving} onCancel={() => setPanel(null)} onSubmit={(form) => mutate(() => api.post(`/quotation-projects/${project.id}/items`, form), "Item added")} />}
          {panel === "supplier" && <SupplierForm key={editingQuote?.id || "new"} items={project.items} initialQuote={editingQuote} saving={saving} onCancel={() => { setPanel(null); setEditingQuote(null); }} onSubmit={(payload) => mutate(() => editingQuote ? api.patch(`/quotation-projects/${project.id}/quotes/${editingQuote.id}`, payload) : api.post(`/quotation-projects/${project.id}/quotes`, payload), editingQuote ? "Quotation updated" : "Supplier quotation added")} />}
          <section className="quote-section"><div className="quote-section-heading"><div><small>Step 1</small><h2>Shared item list</h2><p>Every supplier is measured against these same quantities.</p></div><button className="quote-secondary-button" type="button" onClick={() => setPanel("item")}><Plus size={16} /> Add item</button></div>{!project.items.length ? <div className="quote-inline-empty">Add the items and quantities you need before recording supplier prices.</div> : <div className="quote-request-list">{project.items.map((item) => <div key={item.id}><span><strong>{item.name}</strong><small>{item.quantity} {item.unit}</small></span><button type="button" className="quote-icon-button quote-danger-button" aria-label={`Delete ${item.name}`} onClick={() => { if (window.confirm(`Remove ${item.name} from every quotation?`)) mutate(() => api.delete(`/quotation-projects/${project.id}/items/${item.id}`), "Item removed"); }}><Trash2 size={15} /></button></div>)}</div>}</section>
          <section className="quote-section"><div className="quote-section-heading"><div><small>Step 2</small><h2>Supplier offers</h2><p>Delivery, discounts and excluded tax are included in the landed total.</p></div><button className="feature-primary-button" type="button" disabled={!project.items.length} onClick={() => { setEditingQuote(null); setPanel("supplier"); }}><Plus size={16} /> Add supplier</button></div>{!project.quotations.length ? <div className="quote-inline-empty">No supplier quotations yet.</div> : <div className="quote-offer-grid">{project.quotations.map((quote) => <QuoteCard key={quote.id} quote={quote} lowestId={lowestId} formatCurrency={formatCurrency} saving={saving} onEdit={(value) => { setEditingQuote(value); setPanel("supplier"); }} onDelete={(value) => { if (window.confirm(`Delete the quotation from ${value.supplier}?`)) mutate(() => api.delete(`/quotation-projects/${project.id}/quotes/${value.id}`), "Quotation deleted"); }} onPrefer={(value) => mutate(() => api.patch(`/quotation-projects/${project.id}/quotes/${value.id}/preference`, { preferred: !value.preferred }), value.preferred ? "Supplier choice removed" : `${value.supplier} selected`)} />)}</div>}</section>
          {!!project.quotations.length && <section className="quote-section quote-comparison-section"><div className="quote-section-heading"><div><small>Step 3</small><h2>Compare line by line</h2><p>Green cells show the lowest unit price. They do not choose the supplier for you.</p></div></div><DesktopComparison project={project} lowestId={lowestId} formatCurrency={formatCurrency} /><MobileComparison project={project} formatCurrency={formatCurrency} /></section>}
        </>}
      </main>
    </div>
  </div>;
